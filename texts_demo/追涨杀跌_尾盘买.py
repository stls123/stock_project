...
class TradeBackTest(object):
    ...
    def buy(self, date):
        ...
        b0 = (self.stock_count == 0 and
              ...
              self.single_df['close'].loc[date] > self.single_df['open'].loc[date])  # 没有持仓，且提前筛选为1
        if b0:
            # print(1)
            ...
            buy_price = self.single_df['close'].loc[date]  # 计算买入价
            ...
            text_df.iloc[-1, text_df.columns.get_loc('buy_type')] = '收盘买入'
    ...
    def get_buy_signal(self):
        ...
        b3 = (self.single_df['close'].shift(3) < self.single_df['open'].shift(3))
        b4 = (self.single_df['close'].shift(4) < self.single_df['open'].shift(4))
        b5 = (self.single_df['l_ma3'].shift(1)*1.03 <= self.single_df['l_ma7'].shift(1))
        b6 = ( (self.single_df['open'].shift(2) - self.single_df['close'].shift(2)) >=
              (self.single_df['close'].shift(1) - self.single_df['open'].shift(1))*2 )

        self.single_df['buy_signal'] = np.where(b1 & b2 & b3 & b4 & b5, 1, 0)
