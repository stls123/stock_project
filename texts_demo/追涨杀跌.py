...
class TradeBackTest(object):
    def __init__(self, money, stock_code, old_money, old_stock_money, 止盈, 止损, start_date, end_date):
        ...
        self.stock_code = stock_code  # 股票数量
        ...
        self.indexdf = self.indexdf.loc[start_date:end_date]
        # print(len(self.indexdf))

    def buy(self, date):
        global text_df, buy_date_count
        # todo 买入条件
        sigge_iloc_date = self.single_df.index.get_loc(date)
        index_iloc_date = self.indexdf.index.get_loc(date)
        b0 = (self.stock_count == 0 and
              self.single_df['buy_signal'].loc[date] == 1 and
              self.single_df['high'].loc[date] >= self.single_df['high'].iloc[sigge_iloc_date - 1] and
              buy_date_count[date.strftime('%Y-%m-%d')] <= 3)  # 没有持仓，且提前筛选为1

        # b1 = self.indexdf['close'].iloc[index_iloc_date - 1] > self.indexdf['open'].iloc[index_iloc_date - 1]
        # b2 = self.indexdf['volume'].iloc[index_iloc_date - 1] > self.indexdf['volume'].iloc[index_iloc_date - 2]
        # # 昨日大盘最低点是20日内的低点位置
        # b3 = self.indexdf['low'].iloc[index_iloc_date - 1] <= self.indexdf['low'].iloc[
        #                                                       index_iloc_date - 20:index_iloc_date - 2].min()
        # b5 = self.indexdf['ma3'].iloc[index_iloc_date - 1] * 1.008 <= self.indexdf['ma7'].iloc[index_iloc_date - 1]
        if b0:
    ...
    def sell(self, date):
        ...
        # 止损条件：5个点止损
        # 打印stop——loss涉及的每一个变量的类型
        # print(self.stock_code, date)
        # print('1',type(self.single_df['low'].loc[date]))
        # print('2',type(self.old_stock_money * (100 - self.stop_loss_num) / 100))
        # print('3',type(self.single_df['open'].loc[date] * self.stock_count))
        stop_loss = (self.single_df['low'].loc[date] * self.stock_count) <= \
                    (self.old_stock_money * (100 - self.stop_loss_num) / 100) < \
    ...
    def get_buy_signal(self):
        ...
        # print(len(self.single_df))
        # todo 提前筛选数据，快速度  ==> 考虑换成self.df
        b1 = (self.single_df['close'].shift(2) < self.single_df['open'].shift(2))
        b2 = (self.single_df['close'].shift(1) > self.single_df['open'].shift(1))
        b3 = (self.single_df['l_ma3'].shift(1) * 1.03 <= self.single_df['l_ma7'].shift(1))
        # b4 = ((self.single_df['open'].shift(2) - self.single_df['close'].shift(2)) >=
        #       (self.single_df['close'].shift(1) - self.single_df['open'].shift(1)) * 2)
        # b5 = (self.single_df['ma20'].shift(1) > self.single_df['ma20'].shift(2))
        b6 = (self.single_df['v_ma3'].shift(1) > self.single_df['v_ma7'].shift(1))
        b7 = (self.single_df['low'].shift(1) < self.single_df['open'].shift(1)*1.01)

        self.single_df['buy_signal'] = np.where(b1 & b2 & b3 & b6 & b7, 1, 0)
...
def print_all_profit_info(初始资金, earnings):
    ...
    text_df.to_csv('../text_files/text_df.csv')
    # 显示结果
    print(text_df)
