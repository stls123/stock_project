...
def sell(df: pd.DataFrame, date) -> None:
    ...
    if stock_count != 0:
        if open_stop_loss or open_stop_profit or open_move_loss:  # 开盘止损 开盘止盈 开盘移动止损
            ...
            text_df.iloc[-1, text_df.columns.get_loc('sell_price')] = round((old_stock_money * ((100 - stop_loss_num) / 100)) / stock_count, 2)
            ...
            text_df.iloc[-1, text_df.columns.get_loc('sell_date')] = date.strftime('%Y-%m-%d')  # 记录卖出日期数据
            text_df.iloc[-1, text_df.columns.get_loc('sell_price')] = round((old_stock_money * ((100 + stop_profit_num) / 100)) / stock_count,2)
            ...
            text_df.iloc[-1, text_df.columns.get_loc('sell_date')] = date.strftime('%Y-%m-%d')  # 记录卖出日期数据
            text_df.iloc[-1, text_df.columns.get_loc('sell_price')] = round(df['low'].iloc[df.index.get_loc(date) - 1], 2)
            text_df.iloc[-1, text_df.columns.get_loc('sell_type')] = '止损卖出'
...
def get_buy_signal(stock_code, start_date, end_date):
    ...
    b2 = (df['close'].shift(2) < df['open'].shift(2))
    b3 = df['l_ma3'].shift(1) * 1.07 < df['l_ma7'].shift(1)
    b4 = (df['open'].shift(2) - df['close'].shift(2)) >= (df['close'].shift(1) - df['open'].shift(1)) * 2
    b5 = df['close'].shift(2) * 1.06 < df['open'].shift(2)
    df['buy_signal'] = np.where(b1 & b2 & b4 & b3 & b5, 1, 0)
    return df
