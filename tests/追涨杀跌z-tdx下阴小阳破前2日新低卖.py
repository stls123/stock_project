...
def sell(df: pd.DataFrame, date) -> None:
    ...
    # 移动止损（止损线每天向上移动）
    move_stop = df['low'].loc[date] <= df['low'].iloc[df.index.get_loc(date) - 2] < df['open'].loc[date]
    # 开盘移动止损
    open_move_loss = df['open'].loc[date] <= df['low'].iloc[df.index.get_loc(date) - 2]
    if stock_count != 0:
...
def get_buy_signal(stock_code, start_date, end_date):
    ...
    df['buy_signal'] = np.where(b1 & b2 & b4 & b3, 1, 0)
    return df
